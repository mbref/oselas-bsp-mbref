...
#
# changed copy of ${PTXDIST_TOPDIR}/scripts/install_copy_toolchain.sh
#
...
ptxd_get_lib_path() {
    ...
    lib_path="$(ptxd_cross_cc ${CFLAGS} -print-file-name=${lib})"
...
ptxd_split_lib_prefix_sysroot_eval() {
    local lib_path lib lib_dir lib_mdir prefix tmp pre

    lib_path="${1}"
    pre="${2:+${2}_}"

    lib_mdir="$(ptxd_cross_cc ${CFLAGS} -print-multi-directory)"
    lib_mdir="${lib_mdir##.}"

    lib="${lib_path##*/}"			# the pure library filename "libxxx.so"
    lib_dir="${lib_path%/${lib_mdir:+${lib_mdir}/}${lib}}"	# abs path to that lib
...
_ptxd_get_sysroot_usr_by_sysroot() {
    ...
    sysroot="$(ptxd_cross_cc_v2 ${CFLAGS} | \
...
_ptxd_get_sysroot_usr_by_progname() {
    ...
    prog_name="$(ptxd_cross_cc ${CFLAGS} -print-prog-name=gcc)"
    case "${prog_name}" in
	/*)
